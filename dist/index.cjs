...
const createRule$1 = utils.ESLintUtils.RuleCreator((name) => `moleculer/${name}`);
    ...
    return {
      "CallExpression[callee.name=/wrapService|wrapMixin/] > ObjectExpression": function(node) {
        ...
        for (let i = node.properties.length - 1; i >= 0; i -= 1) {
          ...
          if (field.type === utils.AST_NODE_TYPES.Property && field.key.type === utils.AST_NODE_TYPES.Identifier) {
...
const createRule = utils.ESLintUtils.RuleCreator((name) => `moleculer/${name}`);
    ...
    return {
      'CallExpression[callee.name="wrapService"] > ObjectExpression': function(node) {
        ...
        for (const p of node.properties) {
          if (p.type === utils.AST_NODE_TYPES.Property && p.key.type === utils.AST_NODE_TYPES.Identifier && p.key.name === "mixins" && p.value.type === utils.AST_NODE_TYPES.ArrayExpression) {
            queueWorker = p.value.elements.find(
              (el) => el?.type === utils.AST_NODE_TYPES.CallExpression && el.callee.type === utils.AST_NODE_TYPES.Identifier && el.callee.name === "QueueWorker"
            );
            if (queueWorker) {
              break;
            }
        ...
        for (const p of node.properties) {
          if (p.type === utils.AST_NODE_TYPES.Property && p.key.type === utils.AST_NODE_TYPES.Identifier && p.key.name === "actions" && p.value.type === utils.AST_NODE_TYPES.ObjectExpression) {
            for (const action of p.value.properties) {
              if (action.type === utils.AST_NODE_TYPES.Property && action.value.type === utils.AST_NODE_TYPES.ObjectExpression) {
                const publishedVisibility = action.value.properties.find(
                  (actionP) => actionP.type === utils.AST_NODE_TYPES.Property && actionP.key.type === utils.AST_NODE_TYPES.Identifier && actionP.key.name === "visibility" && actionP.value.type === utils.AST_NODE_TYPES.Literal && actionP.value.value === "published"
                );
